...
def fake_gemini_guidance(*args, **kwargs):
    return AICoachOutput(
        ...
        ai_provider="gemini:test-mock",
        ai_generated=True,
    )


def test_analyze_history_delete_flow(client, monkeypatch):
    monkeypatch.setattr(routes.coach, "generate", fake_gemini_guidance)
    resume_text = b"""
    Alex Student
    ...
    assert payload["ai_provider"] == "gemini:test-mock"
...
def test_analyze_requires_an_ai_key(client):
    resume_text = b"""
    Alex Student
    Email: alex@example.com
    Education Bachelor Computer Science
    ...
    assert "Neither OPENAI_API_KEY, HF_TOKEN, nor GEMINI_API_KEY is configured." in response.json()["detail"]
